...
Q1_values = np.zeros(shape=([2, 20, 20, 20, 3, 28, 3, 100]))
Q2_values = Q1_values
...
def choose_action_2(Q1, Q2, state, epsilon):
        """
        This is the policy to select actions.

        Args:
            action_value_estimates: A numpy array containing the action-value
            estimates for a given bandit problem environment.
            Here state represents the index of the state in the Q_table
        Returns:
        ...
        if roll <= epsilon:
            ...
            Q_Total = Q1[:, state[0], state[1], state[2], state[3], state[4], state[5], state[6]] + Q2[:, state[0], state[1], state[2], state[3], state[4], state[5], state[6]]
            action = np.random.choice(np.where(Q_Total==Q_Total.max())[0])
        return action
...
for episode in range(episodes):
    ...
    for i in range(num_customers):
        ...
        total_reward = 0
        action = choose_action_2(Q1_values, Q2_values, index_state, epsilon)
        ...
        if (i<num_customers-1):
            ...
            if roll_2 <= 0.5:
                Q_nstate = Q1_values[:, index_newstate[0], index_newstate[1], index_newstate[2], index_newstate[3], index_newstate[4], index_newstate[5], index_newstate[6]]
                Q_state = Q1_values[action, index_state[0], index_state[1], index_state[2], index_state[3], index_state[4], index_state[5], index_state[6]]
                Q1_values[action, index_state[0], index_state[1], index_state[2], index_state[3], index_state[4], index_state[5], index_state[6]] += alpha*(reward_ac+gamma*Q_nstate.max()-Q_state)
            else:
                Q_nstate = Q2_values[:, index_newstate[0], index_newstate[1], index_newstate[2], index_newstate[3], index_newstate[4], index_newstate[5], index_newstate[6]]
                Q_state = Q2_values[action, index_state[0], index_state[1], index_state[2], index_state[3], index_state[4], index_state[5], index_state[6]]
                Q2_values[action, index_state[0], index_state[1], index_state[2], index_state[3], index_state[4], index_state[5], index_state[6]] += alpha*(reward_ac+gamma*Q_nstate.max()-Q_state)
        else:
            if roll_2 <= 0.5:
                Q_state = Q1_values[action, index_state[0], index_state[1], index_state[2], index_state[3], index_state[4], index_state[5], index_state[6]]
                Q1_values[action, index_state[0], index_state[1], index_state[2], index_state[3], index_state[4], index_state[5], index_state[6]] += alpha*(reward_ac-Q_state)
            else:
                Q_state = Q2_values[action, index_state[0], index_state[1], index_state[2], index_state[3], index_state[4], index_state[5], index_state[6]]
                Q2_values[action, index_state[0], index_state[1], index_state[2], index_state[3], index_state[4], index_state[5], index_state[6]]+= alpha*(reward_ac-Q_state)
...
plt.xlabel('episodes')
plt.savefig('mean_episodes_DQ_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.png', bbox_inches='tight')
plt.close()

fig, (ax1,ax2) = plt.subplots(1,2, figsize=(15, 5))
ax1.plot(mean_episodes_Q, label='Double Q-learning')
ax1.set_xlabel('episodes')
ax1.set_ylabel('Reward')
ax1.plot(mean_episodes_Random , color="purple", label='Random')
ax1.legend()
ax2.plot(mean_episodes_Q, label='Double Q-learning')
ax2.set_xlabel('episodes')
ax2.plot(mean_episodes_Allincrease, color="purple", label='All increase')
ax2.legend()
plt.savefig('rewards_strategies_DQ_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.png')
plt.close()
...
plt.savefig('Mean_reward_Q_smooth_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.png')
plt.close()

fig, ax1 = plt.subplots(1,1, figsize=(10, 10))
ax1.plot(mean_episodes_Q_smooth, label='Double Q-learning', linewidth=4) 
...
ax1.legend()
plt.savefig('Reward_strategies_smooth_DQ_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.png')

# Comparison of the policies without smotheen
fig, ax1 = plt.subplots(1,1, figsize=(10, 10))
ax1.plot(mean_episodes_Q, label='Double Q-learning', linewidth=4) 
ax1.set_title('Reward of the algorithm')
plt.xlabel('episodes')
ax1.plot(mean_episodes_Random, color="purple", label='Random')
...
ax1.legend()
plt.savefig('Reward_strategies_DQ_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.png')

# Save the Q-Table and the images of the raw reward function

with open('rewards_DQ_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.pkl', 'wb') as f:
    pickle.dump(mean_episodes_Q, f)

with open('rewards_Random_DQ_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.pkl', 'wb') as f:
    pickle.dump(mean_episodes_Random, f)

with open('rewards_Allincrease_DQ_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.pkl', 'wb') as f:
    pickle.dump(mean_episodes_Allincrease, f)

Q1_values = DOK.from_numpy(Q1_values)
Q2_values = DOK.from_numpy(Q2_values)

with open('Q1_values_DQ_sparse_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.pkl', 'wb') as f:
    pickle.dump(Q1_values, f)

with open('Q2_values_DQ_sparse_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.pkl', 'wb') as f:
    pickle.dump(Q2_values, f)
