...
for episode in range(episodes):
    ...
    for i in range(num_customers):
        ...
        if (i<num_customers-1):
            ...
            Q_nstate = Q_values[:, index_newstate[0], index_newstate[1], index_newstate[2], index_newstate[3], index_newstate[4], index_newstate[5], index_newstate[6]]
...
plt.xlabel('episodes')
plt.savefig('mean_episodes_Q_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.png', bbox_inches='tight')
plt.close()

fig, (ax1,ax2) = plt.subplots(1,2, figsize=(15, 5))
ax1.plot(mean_episodes_Q, label='Q-learning')
...
ax2.legend()
plt.savefig('rewards_strategies_Q_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.png')
plt.close()
...
ax1.plot(mean_episodes_Q_smooth, label='Q-learning', linewidth=4) 
...
ax1.legend()
plt.savefig('Reward_strategies_smooth_Q_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.png')

# Comparison of the policies without smotheen
fig, ax1 = plt.subplots(1,1, figsize=(10, 10))
ax1.plot(mean_episodes_Q, label='Q-learning', linewidth=4) 
ax1.set_title('Reward of the algorithm')
plt.xlabel('episodes')
ax1.plot(mean_episodes_Random, color="purple", label='Random')
...
ax1.legend()
plt.savefig('Reward_strategies_Q_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.png')

# Save the Q-Table and the images of the raw reward function

with open('rewards_Q_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.pkl', 'wb') as f:
    pickle.dump(mean_episodes_Q, f)

with open('rewards_Random_Q_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.pkl', 'wb') as f:
    pickle.dump(mean_episodes_Random, f)

with open('rewards_Allincrease_Q_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.pkl', 'wb') as f:
    pickle.dump(mean_episodes_Allincrease, f)

Q_values = DOK.from_numpy(Q_values)

with open('Q_values_Q_sparse_permuted'+str(epsilon)+'_'+str(alpha)+str(episodes)+'.pkl', 'wb') as f:
